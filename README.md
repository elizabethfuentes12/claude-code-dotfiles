...
After pushing this repo, configure these in your GitHub repo settings to maximize discoverability:

**About description** (shows under the repo name on GitHub):
```
Sync your Claude Code dotfiles across machines automatically using Git
```

**Topics** — add these tags in the GitHub repo settings:

`claude-code` `dotfiles` `config-sync` `developer-tools` `shell` `zsh` `git` `ai-tools`
